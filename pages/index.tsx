...
//         {courses.map((course) => (
//           <li key={course.id} className='text-white text-center w-[300px] h-[400px] bg-slate-900 p-5 m-2'>
//             <Link href="../[id]" as={`../[id]/${course.id}` } legacyBehavior>
//               <a>{course.name}</a>
//             </Link>
//           </li>
//         ))}
...
function CourseList() {
  return (
    <div>
      ...
      <ul className='flex flex-row flex-wrap justify-center'>
        {courses.map((course) => (
          <li key={course.id} className='text-white text-center w-[300px] h-[400px]
          rounded-lg bg-slate-900 p-5 m-2 bg-opacity-50 bg-center bg-cover bg-no-repeat' style={{backgroundImage:`url(${course.image})` }}>
            <Link href="/course/[id]" as={`/course/${course.id}`} legacyBehavior>
              <div className='relative h-full w-full'>
                <div className="absolute top-0 left-0 w-full h-full bg-black opacity-50" />
                <div className="absolute top-0 left-0 w-full h-full items-center justify-center">
                  <p className='cursor-pointer text-4xl font-bold pt-10' >{course.name}</p>
                  <p>{course.intro}</p>
                </div>
              </div>
            </Link>
